...
function GuiBuilder(camera_, camPosXStart,camPosYStart,camPosZStart,camLookX, camLookY, camLookZ, updateInfos, world, _mainSim, _mode, _simulationInfo, _activeParticleSystems){
	...
	function addStoppSimulationButton(){
		var StopData = function(){
			this.Stop = function(){simulationInfo.simulationIsRunning = false}
		}
	...
	function addParticleSystemFolder(){
	 	...
	 	for(var i=0; i<activeParticleSystems.length; i++){
	 		...
	 		var settingsObj = {};
	 		thisParticlesystemFolder = allParticleFolder.addFolder(name_ps);
...
// 		
//			// fuer jede textur in einem ps schaltflaeche einabauen
//	 		for(var k=0; k<texturesOfPs.length;k++){
//	 			var jsonObj = texturesOfPs[k];
//	 			var nameOfTexture = jsonObj.name;
//	 			var isRenderToScreen = jsonObj.renderToScreen;
//	 			settingsObj[nameOfTexture] = isRenderToScreen;
//	 			var handlerObject = new OnCheckboxShowTextureHandler(name_ps,upInfo,jsonObj);
//	 			thisParticlesystemFolder.add(settingsObj, nameOfTexture).onChange(handlerObject.handlerFunction);
//	 		}	
	 		...
	 		if(emitterEntriesOfPs.length > 0){
	 			...
	 			for(var j=0; j<emitterEntriesOfPs.length; j++){
					...
					var isGpuEmitter = e instanceof EmitterGpu;
	 				...
	 				var emitterSettings = function() {
	 					this.emitterX = e.positionOfEmitter.x;
	 					this.emitterY = e.positionOfEmitter.y;
	 					this.emitterZ = e.positionOfEmitter.z;
	 					this.emitterVelX = e.baseVelocity.x;
	 					this.emitterVelY = e.baseVelocity.y;
	 					this.emitterVelZ = e.baseVelocity.z;	 					
	 					this.Size = e.baseEmitterSize;
	 					this.Particle_Lifetime = e.baseLifeTime;
	 					this.emitsPerCall= e.numberOfParticlesToEmit;
	 				};
	 				var emitter_Settings = new emitterSettings();
	...
	function addWorldFolder(){
		...
		for(var i=0; i<world.localForces.length; i++){
			...
			var localForceSetting = function(){
				this.localX = posOfForce.x;
				this.localY = posOfForce.y;
				this.localZ = posOfForce.z;
				this.mass = massOfForce;
				this.minDistance = mindistanceOfForce;
				this.pull = localForce.attractionMode == 0;
				this.push = localForce.attractionMode == 1;

			}	

			var lForceSetting = new localForceSetting();
			var localForceHandler = new LocalforceController(localForce, lForceSetting);

			folderForThisForce.add(lForceSetting, "localX", -localForcePosLimit, localForcePosLimit).onChange(localForceHandler.onPositionChangeX);
			folderForThisForce.add(lForceSetting, "localY", 0, localForcePosLimit).onChange(localForceHandler.onPositionChangeY);
			folderForThisForce.add(lForceSetting, "localZ", -localForcePosLimit, localForcePosLimit).onChange(localForceHandler.onPositionChangeZ);
			folderForThisForce.add(lForceSetting, "mass", 0, massLimit).onChange(localForceHandler.onMassChange);
			folderForThisForce.add(lForceSetting, "minDistance", 0, minDistanceLimit).onChange(localForceHandler.onMinDistanceChange);
			folderForThisForce.add(lForceSetting, "pull").listen().onChange(localForceHandler.onPullChange);
			folderForThisForce.add(lForceSetting, "push").listen().onChange(localForceHandler.onPushChange);
